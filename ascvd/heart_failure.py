...
def calculate_10_yr_heart_failure(
    ...
    return _ascvd.calculate_10_yr_heart_failure_rust(
        sex,
        age,
        total_cholesterol,
        hdl_cholesterol,
...
def calculate_30_yr_heart_failure(
    ...
    return _ascvd.calculate_30_yr_heart_failure_rust(
        sex,
        age,
        total_cholesterol,
        hdl_cholesterol,
...
def batch_calculate_10_yr_heart_failure_risk(df: pd.DataFrame) -> list:
    new_df = df.copy()
    return [
        _ascvd.calculate_10_yr_heart_failure_rust(*row)
...
def batch_calculate_30_yr_heart_failure_risk(df: pd.DataFrame) -> list:
    new_df = df.copy()
    return [
        _ascvd.calculate_30_yr_heart_failure_rust(*row)
        for row in new_df.itertuples(index=False)
    ]
